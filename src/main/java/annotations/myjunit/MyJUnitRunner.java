...
public class MyJUnitRunner {
    ...
    public static void main(String[] args) throws ClassNotFoundException {
        ...
        testing(testClazz, test, Before.class);

        testingGeneral(testClazz, test);

        testing(testClazz, test, After.class);

        displayNameTesting(testClazz, test);
    ...
    public static void testing(Class<?> testFromObject, SomethingTest test, Class<? extends Annotation> annotationClass) {
        for (Method method : testFromObject.getMethods()) {
            try {
                if (method.isAnnotationPresent(Test.class) && method.isAnnotationPresent(annotationClass)) {
                    method.invoke(test);
                    System.out.println(method.getName() + " успешно прошел");
                }
            } catch (Throwable e) {
